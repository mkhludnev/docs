...
1. Создайте [ключ шифрования](../../kms/concepts/key.md):
   ...
   - Консоль управления {#console}
     ...
     Вместе с ключом создается его первая версия: выберите ключ в списке, чтобы открыть страницу с его атрибутами.
...
1. Назначьте сервисному аккаунту `win-secret-sa` [роль](../../iam/concepts/access-control/roles.md) `kms.keys.encrypterDecrypter`:
   ...
   - Консоль управления {#console}
     ...
     1. На странице **{{ ui-key.yacloud.common.resource-acl.label_access-bindings-in-iam.serviceAccount }}** найдите аккаунт `win-secret-sa` в списке и нажмите значок ![image](../../_assets/options.svg).
...
1. Создайте секрет в сервисе {{ lockbox-name }} и сохраните в нем логины и пароли пользователей, для которых будут созданы учетные записи в ОС Windows:
